...
## Subsections: Alternative Fine-Tuning Frameworks

The following skills cover specific fine-tuning approaches. This TRL skill is the primary umbrella for LLM post-training; use these for framework-specific details:

### PEFT: Parameter-Efficient Fine-Tuning

For LoRA, QLoRA, and 25+ PEFT methods — the canonical skill when the task is specifically about adapter methods, rank selection, target module configuration, or multi-adapter serving.

**Trigger when:** User asks about LoRA/QLoRA configuration, PEFT methods (AdaLoRA, LoRA+, DoRA, etc.), multi-adapter serving, or reducing trainable parameters below 1%.

**Unique content:**
- LoRA rank/target module selection for different model families
- QLoRA-specific configuration (quantization bit, double quantization, nf4)
- Multi-adapter serving patterns
- 25+ PEFT method comparisons
...
For expert-level GRPO patterns, reward function design philosophy, and production-ready training scripts. This is referenced from the TRL GRPOConfig section above.

**Trigger when:** User asks about GRPO-specific reward shaping, loss behavior analysis, mode collapse detection, adaptive reward scaling, or multi-stage GRPO training.

**Key insight:** GRPO loss starts near 0 and *increases* during training — this is CORRECT. Monitor reward metrics instead of loss for progress.

### Axolotl: YAML-Based Fine-Tuning

For YAML-driven training configs, FSDP configuration, DeepSpeed integration, and multimodal fine-tuning support.

**Trigger when:** User asks about Axolotl-specific YAML keys, FSDP2 configuration, context parallelism, or multimodal training datasets.

**Unique content:**
- YAML config reference for all Axolotl training modes
- FSDP2 YAML configuration (`fsdp_config`, `fsdp_version: 2`)
- DeepSpeed ZeRO stages and offload strategies
- Context parallelism for long sequences
- Multimodal dataset formats (vision-language)

### Unsloth: Fast LoRA Training

For 2-5x faster LoRA/QLoRA training with Unsloth's optimized kernels, gradient checkpointing patterns, and supported model list.

**Trigger when:** User asks about Unsloth-specific optimization, faster training, or memory-efficient training on specific model families (Llama, Mistral, Gemma, Qwen).

**Unique content:**
- Unsloth `FastLanguageModel` API patterns
- Memory optimization for 2-5x speedup
- Gradient checkpointing with `use_gradient_checkpointing="unsloth"`

### PyTorch FSDP: Distributed Training

For Fully Sharded Data Parallel training, CPU offloading, FSDP2, and mixed-precision configuration.

**Trigger when:** User asks about FSDP sharding strategies, CPU offload, FSDP2, or distributed training across multiple GPUs for large models.

**Unique content:**
- FSDP sharding strategies (FULL_STATE_DICT, SHARDED_STATE_DICT)
- CPU offloading with `offload_params`
- FSDP2 vs FSDP1 differences
- Mixed precision (BF16) configuration
- Join context manager for uneven inputs
