...
=== Skill 猎人报告 === 2026-05-30 ===

[推荐1] cipher-market-research
Slug: cipher-market-research @renoblabs
功能：市场情报分析 — 竞品分析、定价研究、市场规模、趋势报告，每条结论均有数据来源。
亮点：MIT-0 免费；提供竞争分析/市场规模/定价分析/趋势报告四种服务；专门针对 EdTech 和企业场景。

[推荐2] sql-database-toolkit
Slug: sql-database-toolkit @sqlskills
功能：全链路 SQL 数据分析工具包 — 数据库连接、SQL 查询、数据可视化（24+静态图表+12种交互图表）、AI 洞察、报告生成。
亮点：支持 SQLite/MySQL/PostgreSQL/ClickHouse 等多数据库；内置 90+ 行业报告模板；自然语言转 SQL。

[推荐3] productivity-bot
Slug: productivity-bot @chinakingkong
功能：自动化效率机器人 — 数据处理（CSV/Excel）、定时任务调度、邮件/Slack/Discord 通知。
亮点：开箱即用的调度器 + 数据清洗管道；适合日常运营自动化。
