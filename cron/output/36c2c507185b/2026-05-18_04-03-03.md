...
=== Skill 猎人报告 === 2026-05-18 ===

[推荐1] productivity-automation-kit
功能：效率自动化工具箱 — 自动化工作流模板 + 日程管理助手 + 任务提醒工具 + 数据整理自动化。
亮点：匹配度最高(3.214)，覆盖日程/任务/数据整理，是 lobster 各部门都需要的效率基础设施。

[推荐2] competitive-intelligence-market-research
功能：B2B SaaS 竞争情报系统，覆盖 24 个场景（Sales/HR/Fintech/Ops Tech）。
亮点：匹配度 3.193，专为市场竞争分析设计，可替代或增强现有的公司调研日报。

[推荐3] agent-browser-assistant
功能：浏览器自动化助手，支持网页数据抓取、表单填写、页面截图、UI 测试。
亮点：匹配度 1.055，是 QA 和内容部门进行网页数据采集的核心工具。
